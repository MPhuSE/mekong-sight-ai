...
export class FarmController {
    ...
    private parseCoordinates(latitude: any, longitude: any) {
    ...
    private buildGeometryPolygon(lat: number, lng: number) {
        const d = 0.0001;
        return `POLYGON((${lng - d} ${lat - d}, ${lng + d} ${lat - d}, ${lng + d} ${lat + d}, ${lng - d} ${lat + d}, ${lng - d} ${lat - d}))`;
    }

    private getDefaultThresholdConfig() {
    ...
    private normalizeThresholdPayload(payload: any, farmType: string) {
        ...
        if (requiresRice && !ricePreset) throw new Error('Giống lúa không hợp lệ.');
        if (requiresShrimp && !shrimpPreset) throw new Error('Giống tôm không hợp lệ.');

        const num = (value: any, fallbackValue: number) => {
    ...
    async updateFarm(req: Request, res: Response) {
        try {
            ...
            if (farm_name !== undefined) updateData.farm_name = String(farm_name).trim();
            if (farm_type !== undefined) updateData.farm_type = farm_type;
            if (area_hectares !== undefined) updateData.area_hectares = area_hectares;
            if (farm_code !== undefined) updateData.farm_code = farm_code ? String(farm_code).trim() : null;
    ...
    async getAlertConfig(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const userId = (req as any).user?.sub;
            ...
            logger.error(`Get alert config error: ${error.message}`);
            return res.status(500).json({ success: false, message: error.message || 'Không thể tải cấu hình ngưỡng.' });
        }
    }

    async updateAlertConfig(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const userId = (req as any).user?.sub;
            ...
            logger.error(`Update alert config error: ${error.message}`);
    ...
    async deleteFarm(req: Request, res: Response) {
        try {
            ...
            const safeDeleteByFarm = async (table: string) => {
            ...
            if (devices && devices.length > 0) {
                ...
                if (readingsError && readingsError.code !== '42P01') throw readingsError;
            }
