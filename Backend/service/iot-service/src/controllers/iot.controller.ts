...
type FarmAlertConfig = {
    rice_warning_max: number;
    rice_critical_max: number;
    shrimp_warning_min: number;
    shrimp_warning_max: number;
    shrimp_critical_min: number;
    shrimp_critical_max: number;
    shrimp_optimal_min: number;
    shrimp_optimal_max: number;
};
...
export class IoTController {
    ...
    private buildTrainLikeSimulatedReading(timestamp: Date, seed: number) {
        ...
        const salinity = this.clamp(
            salinityBase + bias * 0.45 + dayWave * 0.35 + (Math.random() - 0.5) * (isDrySeason ? 0.8 : 0.45),
            0.2,
            8.5
        );
        const temperature = this.clamp(
            29 + dayWave * 1.1 + bias * 0.25 + (Math.random() - 0.5) * 0.55,
            26.0,
            32.8
        );
        const ph = this.clamp(
            7.47 + bias * 0.06 + (Math.random() - 0.5) * 0.2,
            7.1,
            7.85
        );
        const waterLevel = this.clamp(
            (isDrySeason ? 58 : 72) + dayWave * 2.0 + bias * 2.5 + (Math.random() - 0.5) * 4.0,
            45,
            90
        );
        const batteryVoltage = this.clamp(
            4.05 - ((seed % 17) / 1000) - Math.random() * 0.1,
            3.6,
            4.2
        );
     ...
     */
    private async getFarmAlertConfig(farmId: string): Promise<FarmAlertConfig | null> {
        const { data, error } = await this.supabase
            .from('farm_alert_configs')
            .select(
                'rice_warning_max, rice_critical_max, shrimp_warning_min, shrimp_warning_max, shrimp_critical_min, shrimp_critical_max, shrimp_optimal_min, shrimp_optimal_max'
            )
            .eq('farm_id', farmId)
            .maybeSingle();
    ...
    private evaluateSalinityAlert(
        salinityValue: number,
        isRiceSeason: boolean,
        config: FarmAlertConfig | null
    ): SalinityAlertDecision {
    ...
    async handleReading(request: any, reply: any) {
        try {
            ...
            let readingTimestamp: string | undefined;
     ...
     */
    async getFarmReadingsHistory(request: any, reply: any) {
        try {
            ...
            const deviceIds = deviceRows.map((item: any) => item.id);
            ...
            const enriched = (readings || []).map((row: any) => {
     ...
     */
    async seedSimulatedHistory(request: any, reply: any) {
        try {
            ...
            logger.error(`Seed simulated history error: ${error.message}`);
            return reply.status(500).send({ success: false, message: error.message });
        }
    }
    ...
    async deleteDevice(request: any, reply: any) {
        try {
            ...
            const { error: deleteReadingsError } = await this.supabase
                ...
                .delete()
                .eq('device_id', id);
            if (deleteReadingsError) throw deleteReadingsError;
